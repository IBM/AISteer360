...
from aisteer360.algorithms.core.internals.probes import ProbeSet, ProbeSetFit, RoutingRules
...
class RoutedDecodingArgs(BaseArgs):
    ...
    Attributes:
        probes: The probes whose decisions drive routing. Either a fitted `ProbeSet`, or a
            ...
            provides.
        rules: The `RoutingRules` mapping probe decisions to actions. Probe names are validated
            here against `probes.names` (available in both forms), so misconfigured rules fail
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.rules, RoutingRules):
            raise TypeError(
                f"rules must be a RoutingRules instance; got "
